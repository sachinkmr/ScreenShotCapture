...
public class SiteBuilder extends Site {
	...
	public String setUrlWithAuth(String add) {
		...
		if (this.hasAuthentication()) {
			...
			String path = add.replaceFirst(protocol, "");
			urlauth = protocol + username + ":" + password + "@" + path;
		} else {
			urlauth = url;
		}
		return urlauth;
	...
	private void initialize() throws Exception {
		...
		try {
			...
			RequestConfig requestConfig = RequestConfig.custom().setRedirectsEnabled(true)
					...
					.setConnectionRequestTimeout(1000000).setSocketTimeout(1000000).setConnectTimeout(1000000).build();
			CloseableHttpClient httpclient = null;
			if (this.hasAuthentication()) {
				credentialsProvider.setCredentials(AuthScope.ANY,
						new UsernamePasswordCredentials(getUsername(), getPassword()));
				httpclient = HttpClients.custom().setDefaultRequestConfig(requestConfig)
						.setUserAgent(this.getUserAgent()).setDefaultCredentialsProvider(credentialsProvider).build();
			} else {
				httpclient = HttpClients.custom().setDefaultRequestConfig(requestConfig)
